...
function initializeLegacyTransition() {
  const isEmbeddedPreview = new URLSearchParams(window.location.search).get('embed') === '1';
  const transitionLinks = document.querySelectorAll('#legacyToModern, #legacyToModernHero');

  if (isEmbeddedPreview) {
    const strip = document.querySelector('.legacy-transition-strip');
    if (strip) {
  ...
  transitionLinks.forEach((link) => {
    link.addEventListener('click', (event) => {
      ...
      sessionStorage.setItem('nexas_from_legacy', '1');
      document.body.classList.add('page-transitioning');

      window.setTimeout(() => {
        window.location.href = '../index.html?from=legacy';
      }, 620);
    });
  });
}
